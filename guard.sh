...
# 仓库地址：https://github.com/auth-server-by/auth-server-by
# ==================================================
...
showProcessMenu() {
    ...
    for i in "${!processes[@]}"; do
        IFS=':' read -r pid name <<< "${processes[$i]}"
        echo -e "\033[1;32m$((i+1)). PID: $pid | $name\033[0m"
    done
    echo -e "\033[1;35m----------------------------------------\033[0m"
    ...
    IFS=':' read -r targetPid targetName <<< "${processes[$index]}"
